...
var_min <- var_seconds %/% 60
var_sec <- var_seconds %% 60

if (var_min == 1 && var_sec == 1) {
  cat(var_seconds, "seconds is", var_min, "minute and", var_sec, "second \n")
} else if (var_sec == 1) {
  cat(var_seconds, "seconds is", var_min, "minutes and", var_sec, "second \n")
} else if (var_min == 1) {
  cat(var_seconds, "seconds is", var_min, "minute and", var_sec, "seconds \n")
} else {
  cat(var_seconds, "seconds is", var_min, "minutes and", var_sec, "seconds \n")
}
...
cat("The largest number is:", largest_num, "\n")
...
while (counter <= fact_num){
  fact_value <- fact_value * counter
  counter <- counter + 1
}
...
factors_of_my_num <- integer(0)

for (num in (1:my_num)) {
  if (my_num %% num == 0) {
    factors_of_my_num <- c(factors_of_my_num, num)
  }
}

cat(factors_of_my_num, sep = ", ")
...
if (potential_prime > 1) {
  is_prime <- TRUE
  for (num in 2: (potential_prime - 1)) {
...
if (is_prime) {
  cat(potential_prime, "is a prime number", "\n")
} else {
  cat(potential_prime, "is not a prime number", "\n")
}
...
for (num in (1:my_var)) {
      num_sqr <- num**2
      if (num_sqr > my_var) {
        cat(num, "squared is", num_sqr, "and it is the first number whose square exceeds", my_var)
        break
      }
}
