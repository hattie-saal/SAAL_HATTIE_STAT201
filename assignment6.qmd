...
```{r}
cat("bears:", append(bears, 40, after=4), "\n")

cat("opponent:", append(opponent, 38, after=0), "\n")
...
```{r}
spread_result <- bears - opponent 
index_value <- which.max(spread_result)
bears_value <- bears[index_value]
opponent_value <- opponent[index_value]

cat("The largest win spread is", max(spread_result), "when the Bears scored", bears_value, "points and the Opponent scored", opponent_value, "points")
...
new_row <- c(37, 17)
new_row_2 <- c(9, 17)
game_score <- rbind(game_score, new_row, new_row_2)
print(game_score)
...
for (i in 1: nrow(game_score)){
  win[i] <- game_score[i, 1] > game_score[i, 2]
}
...
for (i in 1:nrow(game_score)) {
  for (j in 1:ncol(game_score)) {
    if (game_score[i, j] == "FALSE") {
      counter <- counter + 1
    } else if (game_score[i, j] == "TRUE") {
      found_true <- TRUE
      break
    }
  } 
...
```{r}
number_of_factors <- function(number) {
  num_of_factors <- numeric(0)
  for (i in 1:number) {
    if (number %% i == 0) {
      num_of_factors <- c(num_of_factors, i)
    }
  }
  return (length(num_of_factors))
}
...
rectangle_area <- function(length=48, width=48) {
  in_sq_in <- length * width
  print(paste("The area in square inches is", in_sq_in))
  in_sq_ft <- (length / 12) * (width / 12)
  print(paste("The area in square feet is", in_sq_ft))
}

rectangle_area_calculation <- rectangle_area(102, 60)
...
```{r}
word_with_most_letters <- function(string_vector) {
  longest_string_vector <- character(0L)
  lengths <- nchar(string_vector)
  max_length <- max(lengths)
  longest_strings <- string_vector[lengths == max_length]
  longest_string_vector <- c(longest_string_vector, longest_strings)
  return (longest_string_vector)
}
word_with_most_letters(fruits)
```
...
```{r}
check_in_vector <- function(value, vector) {
  for (i in vector) {
    if (value == i) {
      return (TRUE)
    } else {
...
output_var <- check_in_vector(value, check)
...
```{r}
matrix_converter <- function(matrix) {
  matrix[matrix < 0] <- NA
  return (matrix)
}

new_matrix <- matrix_converter(x)
print(new_matrix)
```
...
matrix_converter_2 <- function(matrix) {
